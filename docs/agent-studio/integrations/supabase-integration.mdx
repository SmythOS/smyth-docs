...
* **Build an Agent That...**
    ...
    * Creates a logging service. After any critical action (like sending an email with [Resend](/docs/integrations/resend) or processing a payment with [Stripe](/docs/integrations/stripe-integration)), an agent inserts a log record into a `logs` table in Supabase.
