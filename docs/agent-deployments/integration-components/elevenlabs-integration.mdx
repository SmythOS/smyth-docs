...
- Optionally configured Vault access in [Vault → API Keys](/docs/agent-studio/key-concepts/vault) for managing secrets securely
...
<InfoCallout>
For secure storage and reuse, we recommend saving your API key in [Vault → API Keys](/docs/agent-studio/key-concepts/vault) and to not share it with others.
</InfoCallout>
...
- Monitor audio request patterns in [Agent Logs](/docs/agent-studio/manage-agents/logs)
