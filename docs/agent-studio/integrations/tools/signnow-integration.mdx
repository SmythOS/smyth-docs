...
og:image: /img/integrations/smythos_signnow.png
...
|---------------|------------|------------------|------------|------------------|---------------|
| [**Send for Signature**](#send-for-signature)| Write | Sends a document from your SignNow account to a recipient to be signed. | <Badge type="required" /> `document_id`, `to`, `from`, `subject` | `id` | Sending a sales contract to a new client for their signature. |
...
|-----------------|--------|--------------------|---------------|
| **Send a document out for signature** | A `document_id` and a recipient's email | [Send for Signature](#send-for-signature) | This is the core component for initiating the e-signature process for a document. |
...
### <a name="send-for-signature"></a>Send for Signature
...
* **Explore Other Integrations:**
    * Combine SignNow with a database integration like [Firestore](/docs/integrations/firestore). When a document is sent for signature, log the invitation `id` and status in a Firestore document for auditing.
    * Use an [LLM component](/docs/integrations/openrouter) to generate a custom legal clause, add it to a contract template, and then send it for signature with SignNow.
    * When a contract is signed, trigger an agent to create a new project in [Asana](/docs/integrations/asana) and a new customer record in [Stripe](/docs/integrations/stripe).
