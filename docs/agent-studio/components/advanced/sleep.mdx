...
- **Manage Rate Limits:** This is the most common use case. Place a `Sleep` component inside a **[ForEach Loop](../logic/for-each)** when calling an API to ensure you don't exceed the service's request limit.
...
- Place a `Sleep` inside a **[ForEach Loop](../logic/for-each)** that makes an **[API Call](../base/api-call)** on each iteration to process a list without getting rate-limited.
