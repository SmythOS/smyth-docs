...
# How to Debug Agents in Weaver

> **Agents break.** Network blips, bad API keys, hallucinating LLMs — life happens.  
> Weaver’s debugging tools keep those hiccups from becoming hair-pullers.

<InfoCallout> **TL;DR:** Click the red component, read the I/O snapshot, hit **Fix with AI**, rerun. 80 % of issues disappear in less than 30 seconds. </InfoCallout>
...
- **Watch run logs** — <DocLink to="../agent-studio/studio-settings/logs">agent activity logs</DocLink> surface slow steps early.  
> **Red borders happen.**  
...
## The `_error` Output Pattern

Every skill can emit an `_error` object:

```json
{
  "issue": "WordPress 403 — auth failed",
  "suggested_fix": "Refresh OAuth token or check user role",
  "raw_response": { "status": 403, "body": "Forbidden" }
}
``` 
Weaver routes _error to downstream Debug Analyzer blocks so you can auto-remediate.

<PromptCard prompt={"If status === 403 then prompt user to connect WordPress again"} />

<Divider/>

Use the timeline view to spot >1 s lag spikes as they’re your bottlenecks.
