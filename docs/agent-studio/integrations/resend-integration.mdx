...
og:image: /img/integrations/smythos_resend.png
...
* **Build an Agent That...**
    ...
    * Sends a daily summary report. The agent gathers data from various sources ([Google Analytics](/docs/integrations/google-analytics), internal databases), formats it into an HTML report, and emails it to stakeholders.
...
* **Explore Other Integrations:**
    * Combine Resend with a database integration like [Firestore](/docs/integrations/firestore). When a new record is added, trigger an agent to send a confirmation email.
    * Use an [LLM component](/docs/integrations/openrouter) to draft a personalized follow-up email based on a customer's recent activity, then send it via Resend.
