...
- Your agent must be publicly accessible (e.g., [Deploy via Webhook](/docs/agent-deployments/deploy-as-webhook))
- iPhone or iPad with iOS 14+
- The **Shortcuts** app installed (usually default on iOS)

<Divider />

## Step-by-Step: Connect Siri to SmythOS

### Step 1: Copy Your Webhook URL

1. Go to your deployed agent in SmythOS
2. Click **Deploy** and select **Webhook**
3. Copy the generated URL (it will look like `https://your-agent.smythos.com/api/v1/chat`)
4. If needed, configure authentication using a **Bearer Token** or set up [Vault access](/docs/agent-collaboration/working-with-agents/vault-sharing)

<InfoCallout>
You can test this webhook first using [Postman](https://www.postman.com/) or `curl` to ensure it responds correctly before adding it to Siri.
</InfoCallout>
...
### Step 3: Trigger It with Your Voice
...
- [Log all invocations](/docs/agent-studio/agent-settigns/logs) in SmythOS to track Siri activity
