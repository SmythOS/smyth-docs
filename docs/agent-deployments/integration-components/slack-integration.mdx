...
<InfoCallout>
**TL;DR:** Authenticate your Slack app, configure the Send Message component, or deploy using [Model Context Protocol (MCP)](/docs/agent-deployments/deployments/deploy-as-mcp) to route intelligent agent outputs into Slack.
</InfoCallout>

<Divider />

## Why Integrate Slack with SmythOS?
...
- Optional: Store the token in [Vault → API Keys](/docs/agent-collaboration/working-with-agents/vault-sharing)
...
If your agent uses memory, roles, or multi-modal workflows, deploy with [MCP](/docs/agent-deployments/deployments/deploy-as-mcp) to:
...
- Use [Vault](/docs/agent-collaboration/working-with-agents/vault-sharing) to manage tokens securely
- Track delivery and failures using [Agent Logs](/docs/agent-studio/agent-settings/logs)
