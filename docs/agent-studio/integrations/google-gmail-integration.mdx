...
- You will need to generate **OAuth 2.0 credentials (`Client ID` and `Client Secret`)** from the Google Cloud Console. This process involves configuring an OAuth consent screen and enabling the Gmail API. Detailed steps are provided below.
- The **Authorized redirect URI** for SmythOS:  
...
Connecting Gmail to SmythOS is a one-time setup. You’ll first create secure credentials in Google Cloud, then bring those into SmythOS.  

### Part A: Create Your Google OAuth Credentials

#### 1. Open Google Cloud Console
- Visit [Google Cloud Console](https://console.cloud.google.com/).
- Sign in with the Google account that owns the Gmail inbox you want to connect.
- Choose an existing project or create a new one for this integration.
...
- A short wizard will guide you through setup:

**App Information**  
- Enter an app name (e.g., `SmythOS Gmail Integration`)  
- Add a user support email  

**Audience (User Type)**  
- **Internal** → restricts to your Workspace users  
- **External** → allows any Google account (requires test users while in Testing mode)  

**Developer Contact Information**  
- Provide your email for Google notifications  

Click **Create** to finish.

<InfoCallout title="Testing Mode">
If you chose **External**, Google puts your app in *Testing* mode.  
You must return to **Audience → Edit test users** and add the Gmail addresses that will be allowed to connect.
</InfoCallout>
...
- Fill in:  
  - **Name:** e.g., `SmythOS Gmail Web Client`  
...
<WarningCallout title="Exact Match Required">
The redirect URI must match **exactly**. Even a trailing slash or typo will block the connection.
</WarningCallout>

#### 5. Copy Your Credentials
- A dialog will display your **Client ID** and allow you to **download JSON**.  
- To get the **Client Secret**, click into your client details.  

**Important Change:** Google no longer allows re-viewing secrets once you leave the page.  
If you lose it, you must **create a new secret** from the Client details page.

<TipCallout title="Secure Your Credentials">
Treat your `Client ID` and `Client Secret` like passwords.  
We recommend storing them in the [SmythOS Vault](/docs/agent-studio/key-concepts/vault) for safe management.
</TipCallout>

### Part B: Connect Gmail in SmythOS

1. In your SmythOS agent [Canvas](/docs/agent-studio/key-concepts/canvas), go to **Integrations** in the left sidebar.  
2. Find **Google Gmail** and click **Add / Configure** on the component(s) you want.  
3. Paste in your **Client ID** and **Client Secret**.  
4. **Save your settings**:  
   - Close the sidebar (this ensures the fields are saved).  
   - Then either:  
     - Reopen Gmail settings and click **Authenticate**, **or**  
     - Simply click the blue **Authenticate** button that appears below the Gmail component.  
5. A **Google Sign-In** window will open:  
   - Log in with the Gmail account you want to connect  
   - Review the requested permissions  
   - Click **Allow**  
6. You’ll be redirected back to SmythOS, and the Gmail integration will update.

<SuccessCallout title="Connection Confirmed!">
Your Gmail account is now successfully linked with SmythOS.  
From here on, agents in SmythOS can securely send or read emails according to the scopes you granted.
</SuccessCallout>

### Optional: Fine-Tune Branding & Scopes

**Branding options:**  
- Upload a logo (120×120px, JPG/PNG/BMP, less than 1MB)  
- Add app domain links (home page, privacy policy, ToS)  
- Register **Authorized domains** (required if your links or URIs use them)  
- Add developer contact emails  

**Scopes (Data Access):**  
Choose the minimum access SmythOS needs. For example, if SmythOS only needs to send emails, `gmail.send` is all you need:  

- `https://www.googleapis.com/auth/gmail.send` → send emails  
- `https://www.googleapis.com/auth/gmail.readonly` → read emails  
- `https://www.googleapis.com/auth/gmail.modify` → read, send, delete (non-permanent)  
- `https://mail.google.com/` → full access (**use only if absolutely necessary**)  

<InfoCallout title="Need to Switch or Reconnect?" collapsible>
If you want to connect a different Google account or refresh the integration, use the **Disconnect** or **Deauthenticate** option in SmythOS Gmail settings. Then repeat the authentication steps above.
</InfoCallout>
