...
| **Key/Secret** | Define the cryptographic key to use for signing. For **HMAC**, this is a secret string. For **RSA**, this must be a valid RSA private key (e.g., in PEM format). <TipCallout title="Secure Your Keys">Store your keys and secrets in the **[Vault](/docs/agent-collaboration/working-with-agents/vault)** and reference them here as variables (e.g., `{{vault.hmac_secret}}`).</TipCallout> |
