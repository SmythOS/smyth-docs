...
* **Build an Agent That...**
    * Generates monthly invoices. The agent fetches billing data from [Stripe](/docs/agent-studio/integrations/stripe-integration), populates an invoice template using `Generate PDF`, and then emails it to the customer using [SendGrid](/docs/integrations/sendgrid).
...
* **Explore Other Integrations:**
    ...
    * Use a [web scraper](/docs/agent-studio/integrations/scrapingbee-integration) to gather data, format it into an HTML report, and then use the `HTML to PDF` component from [PdfCrowd](/docs/integrations/pdfcrowd) or a similar tool to create a final document if you don't have a template.
