...
og:image: /img/integrations/smythos_asana.png
...
* **Build an Agent That...**
    ...
    * Syncs with your calendar. The agent reads your [Microsoft Calendar](/docs/integrations/microsoft-calendar) and creates Asana tasks for all upcoming meetings with a specific client.

* **Explore Other Integrations:**
    * When a task is marked as "complete" in Asana, trigger an agent to send a notification to a [Microsoft Teams](/docs/integrations/microsoft-teams) or [Slack](/docs/integrations/slack) channel.
    * Use an [LLM component](/docs/integrations/openrouter) to summarize a long email thread, and then create a new task in Asana with the summary in the notes.
    * When a new row is added to a [Google Sheet](/docs/integrations/google-sheets), create a corresponding task in an Asana project.
