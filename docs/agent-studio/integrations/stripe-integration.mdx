...
* **Build an Agent That...**
    ...
    * Handles failed payments. When a subscription payment fails (triggered by a Stripe webhook), an agent sends a notification to the customer via SMS using [Twilio](/docs/integrations/twilio) and creates a follow-up task in [Asana](/docs/agent-studio/integrations/asana-integration) for the customer success team.
