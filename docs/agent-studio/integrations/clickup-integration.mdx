...
og:image: /img/integrations/smythos_clickup.png
...
* **Build an Agent That...**
    ...
    * Syncs with your calendar. The agent reads your [Microsoft Calendar](/docs/integrations/microsoft-calendar) and creates a ClickUp task for every meeting scheduled for the next day.

* **Explore Other Integrations:**
    * When a task is updated to "In Review" in ClickUp, trigger an agent to send a notification to a [Microsoft Teams](/docs/integrations/microsoft-teams) channel.
    * Use an [LLM component](/docs/integrations/openrouter) to summarize a long project brief, then create a new parent task in ClickUp with the summary as the description.
