...
- **Manage Rate Limits:** This is the most common use case. Place a `Sleep` component inside a **[ForEach Loop](/docs/agent-studio/components/logic/for-each)** when calling an API to ensure you don't exceed the service's request limit.
...
- **Avoid Very Long Delays:** For pauses longer than a few minutes, it's better to use an **[Async](./async)** and **[Await](./await)** pattern. The `Sleep` component is designed for short-term delays.
...
- Place a `Sleep` inside a **[ForEach Loop](/docs/agent-studio/components/logic/for-each)** that makes an **[API Call](/docs/agent-studio/components/base/api-call)** on each iteration to process a list without getting rate-limited.
- Use a **[Code Component](./code)** to calculate a dynamic delay based on business logic, and pass the result to the `Sleep` component.
- Create a more human-like chatbot by using a `Sleep` component to add a pause before the **[LLM Assistant](./llm-assistant)** generates a response.
