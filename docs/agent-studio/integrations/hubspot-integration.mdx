...
og:image: /img/integrations/smythos_hubspot.png
...
* **Build an Agent That...**
    ...
    * Syncs data from your e-commerce platform. When a new customer makes a purchase on [Shopify](/docs/integrations/shopify), the agent finds or creates a `Contact` in HubSpot and updates a custom property with their total order value.
    * Automates support. It receives an email, uses an [LLM](/docs/integrations/openrouter) to determine if it's a bug report, and if so, creates a new `Ticket` in the Service Hub.

* **Explore Other Integrations:**
    * When a new contact is created, trigger a workflow to add them to a specific marketing list in [Mailchimp](/docs/integrations/mailchimp) or [Klaviyo](/docs/integrations/klaviyo).
    * When a deal is marked "Closed-Won," trigger an agent to create a new client record in your accounting software and a new project in [Asana](/docs/integrations/asana).
    * Use the [Twilio Integration](/docs/integrations/twilio) to send an SMS notification to a sales rep when a high-priority deal is created for them.
