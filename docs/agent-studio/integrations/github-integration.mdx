...
- For security, store this token in the SmythOS **Vault**. Create a new secret named `github_pat` and paste your token as the value. [Learn more about the Vault here.](/docs/agent-collaboration/working-with-agents/vault)
