...
og:image: /img/integrations/smythos_inflow.png
...
* **Build an Agent That...**
    * Acts as a full product lifecycle manager. When a new product is added to a [Google Sheet](/docs/agent-studio/integrations/google-sheets-integration), the agent creates it in InFlow, then creates a corresponding product page on a [Shopify](/docs/integrations/shopify) or [Squarespace](/docs/integrations/squarespace) store.
    ...
    * Syncs customer data. When a new client is added in [Jobber](/docs/integrations/jobber), an agent creates a corresponding customer record in InFlow Inventory.
...
* **Explore Other Integrations:**
    ...
    * Use an [LLM component](/docs/integrations/openrouter) to generate a rich product description, then use the InFlow API to update the product record.
