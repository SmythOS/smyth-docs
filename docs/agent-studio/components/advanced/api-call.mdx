...
<InfoCallout title="Why this matters">You can reach nearly any REST-style service without custom code. <br/>Define your call once, pass dynamic inputs at runtime, and inject secrets via [Vault](/docs/agent-collaboration/working-with-agents/vault-sharing).</InfoCallout>
...
- [Handle Pagination and Rate Limits](#step-4-handle-pagination-and-rate-limits)
...
<InfoCallout>
Store API keys in [Vault](/docs/agent-collaboration/working-with-agents/vault-sharing) and inject them like `{{vault.api_key}}` inside headers or body.
</InfoCallout>
...
<Steps>
  <li>Toggle <strong>Use Mock Data</strong> in the settings panel</li>
  <li>Edit the mock response JSON in the editor</li>
  <li>Use <strong>Debug</strong> to simulate how your agent will behave</li>
</Steps>
...
<Image
  src="/img/components/mock-editor.png"
...
- Never hardcode credentials — use [Vault](/docs/agent-collaboration/working-with-agents/vault-sharing)  
...
<Image
  src="/img/components/api-call-ui.png"
...
- Combine **[API Call](../advanced/api-call)** with **[Code Component](../advanced/code)** to clean or validate responses  
- Link with **[Agent Skill](../base/agent-skill)** to allow natural inputs (text, voice) to trigger your call  
- Use **Condition blocks** to handle branches like `status === 404`  
- Store secrets in **[Vault](../../docs/agent-collaboration/working-with-agents/vault-sharing)** securely  
