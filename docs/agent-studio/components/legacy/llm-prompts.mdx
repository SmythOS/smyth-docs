...
This is a versatile component for single-turn AI generation. For multi-turn, stateful conversations that require memory, consider using the **[LLM Assistant Component](./llm-assistant)**. For the most up-to-date features, see the **[GenAI LLM Component](../base/gen-ai-llm)**.
</InfoCallout>
...
You can add credentials for other model providers in the **[Vault](/docs/agent-collaboration/working-with-agents/vault-sharing)** to expand your model selection.
</TipCallout>
...
- Use a **[Classifier Component](../base/classifier)** to determine user intent, then route to different `LLM Prompt` components with specialized prompts.
- Pass the output to a **[JSON Filter Component](./json-filter)** to further clean or simplify the data before using it in other steps.
