...
- **Behavior:** A description of what the subagent does. This is pre-filled from the subagent's `Behavior` field but can be edited. This text helps the parent agent's AI understand the subagent's capabilities.
