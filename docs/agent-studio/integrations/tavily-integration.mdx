...
<InfoCallout>This component requires an `API Key` for authentication, as detailed in the [Getting Started](#getting-started-with-tavily) section.</InfoCallout>
...
<WarningCallout title="LLM-Optimized">
...
### <a name="news-search-components"></a>News Search Components
Includes **Basic News Search** and **Advanced News Search**. These components are specifically tuned to search only news sources.

<InfoCallout>These components require an `API Key` for authentication, as detailed in the [Getting Started](#getting-started-with-tavily) section.</InfoCallout>
...
```
<WarningCallout title="Focus on News">
...
<WarningCallout title="Content Extraction">
...
* **Explore Other Integrations:**
    * Combine Tavily with the [OpenRouter](/docs/integrations/openrouter) or [Perplexity AI](/docs/integrations/perplexity-ai) integrations to create state-of-the-art RAG pipelines.
    * Send the results of your Tavily searches directly to a [Google Sheet](/docs/integrations/google-sheets) for logging and analysis.
    * Use the `Extract` component to get clean article text and then pass it to the [PlayHT](/docs/integrations/playht) or [Elevenlabs](/docs/integrations/elevenlabs) integrations to create an audio version.
