...
Weaver is your **chat-first builder** inside [Agent Studio](/docs/agent-studio/overview). Instead of wiring everything manually, you describe what you want, and Weaver creates it on the [Studio Canvas](/docs/agent-studio/canvas) in real time.  
...
If you want deeper control later, you can always switch back to manual edits in the [Canvas](/docs/agent-studio/canvas). Weaver gets you started fast, Canvas helps you fine-tune.
...
7. **Deploy to Runtime**  
   When it works, deploy your agent through [Runtime](/docs/smyth-runtime/overview) as an API or web agent.  

<TipCallout title="Skip ahead with templates">
Start with a [prebuilt template](/docs/agent-weaver/prebuilt-templates). Open it in Studio, then chat with Weaver to adapt it to your needs.
</TipCallout>
...
| Zero-code deployment | Publish directly to [Runtime](/docs/smyth-runtime/overview) |
...
Yes. Weaver builds workflows entirely from chat. For fine control, refine them in the [Studio Canvas](/docs/agent-studio/canvas).  

<br />

**How is Weaver different from Canvas?**  
Weaver is built into Studio as a tab. You chat on the left, and the [Canvas](/docs/agent-studio/canvas) updates live on the right. Canvas gives you manual control; Weaver speeds up the start.  
...
- Explore [prebuilt templates](/docs/agent-weaver/prebuilt-templates) to see ready-to-run examples.  
- Learn the available [components](/docs/agent-studio/components) so you know your building blocks.  
- Switch to the [Canvas](/docs/agent-studio/canvas) for detailed edits and advanced workflows.  
- Deploy through [Runtime](/docs/smyth-runtime/overview) once your agent is ready.  
