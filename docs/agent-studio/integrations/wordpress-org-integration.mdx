...
<InfoCallout title="Heads-up">You must add the Base64 encoded secret from the Vault to each WordPress.org component you use that performs a write action (Create, Update, Delete).</InfoCallout>
<Divider />
...
Creates a new post in a `draft` status on your WordPress.org site, including SEO metadata.

<InfoCallout>This component requires Base64 encoded credentials for authentication, as detailed in the [Getting Started](#getting-started-with-wordpressorg) section.</InfoCallout>

#### Inputs
| Field | Type | Required | Notes |
|-------|------|----------|-------------|
| `domain_name` | string | Yes | The full URL of your WordPress.org site. |
| `title` | string | Yes | The title of the new draft post. |
| `content` | string | Yes | The body content of the post, which can be plain text or HTML. |
| `meta_description`| string | Optional| A brief SEO summary of the post. |
| `keywords` | string | Optional| Comma-separated SEO keywords. |
| `categories`| integer | Optional| The WordPress category ID(s) for the post. |

#### Outputs
| Field | Type | Description |
|-------|------|-------------|
| `id` | integer| The unique ID of the newly created draft post. |
| `Response`| object | The full, raw JSON response from the WordPress REST API. |
...
Creates a new draft post and associates it with a featured image from your WordPress Media Library.
...
| `content` | string | Yes | The body content of the post (HTML or plain text). |
| `image_id` | string | Yes | The ID of an image that is already uploaded to your WordPress Media Library. |
...
Edits the content of an existing post on your site.
...
|-------|------|----------|-------------|
| `domain_name` | string | Yes | The URL of your WordPress.org site. |
| `post_id` | string | Yes | The unique ID of the post to update. |
| `content` | string | Yes | The new HTML or text content for the post. |
...
Includes **List Posts** and **Get Post**. These components retrieve post data from your site.

<InfoCallout>These components do not require authentication as they access publicly available data.</InfoCallout>

#### Common Inputs
| Field | Type | Required | Notes |
|-------|------|----------|-------------|
| `domain_name`| string | Yes | The URL of your WordPress.org site. |
| `post_id` | string | (For Get Post) Yes | The unique ID of the post to retrieve. |
| `per_page` | integer| (For List) Optional | The number of posts to retrieve. Default: `10`. |

#### Outputs
| Field | Type | Description |
|-------|------|-------------|
| `posts` | array | (List Posts) An array of post objects. |
| `title`, `content`, `link` | any | (Get Post) Specific details of the retrieved post. |
| `Response`| object | The raw JSON response from the API. |

<TipCallout title="Use Case">
A "content audit" agent uses `List Posts` to get all articles, then loops through them, using `Get Post` to fetch the full content for analysis or backup.
</TipCallout>
...
* **Build an Agent That...**
    * Connects to your [GitHub repository](/docs/integrations/github). When you merge a new Markdown file into a specific folder, the agent automatically converts it to HTML and creates a new draft post on your WordPress site.
    * Generates a weekly newsletter. The agent uses `List Posts` to get the titles and links of all articles published in the last 7 days, formats them into an HTML email, and sends it using [Resend](/docs/integrations/resend).
    * Acts as a content translation pipeline. It takes an existing English post, sends the content to the [Google Translate Integration](/docs/integrations/google-translate), and then creates a new draft of the post in Spanish.

* **Explore Other Integrations:**
    * Use an [LLM component](/docs/integrations/openrouter) to expand a short idea into a full blog post, then use `Create Draft Post` to save it for review.
    * After an agent creates a new post, it can take the post's title and use the [Ayrshare Integration](/docs/integrations/ayrshare) to announce it on your social media channels.
