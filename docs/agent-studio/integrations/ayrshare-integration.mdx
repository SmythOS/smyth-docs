...
* **Build an Agent That...**
    * Creates a "content machine." It uses an AI model from [OpenRouter](/docs/agent-studio/integrations/openrouter-integration) to write a short social media tip, generates a relevant background image with [Stability AI](/docs/integrations/stability-ai), and posts the final image and text to Instagram and LinkedIn.
