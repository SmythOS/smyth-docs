...
- Use `ForEach` to process records from a **[Database Query](../data-and-storage/database-query)**, updating each one.
- Combine `ForEach` with an **[API Call](../advanced/api-call)** to enrich each item in a list with data from an external service.
- Feed the `Result` of a `ForEach` loop into a **[GenAI LLM Component](../ai-and-machine-learning/genai-llm)** to generate a summary of the processed items.
