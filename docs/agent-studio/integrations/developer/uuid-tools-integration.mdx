...
og:image: /img/integrations/smythos_uuid_tools.png
...
* **Build an Agent That...**
    * Creates new user records in a [Firebase](/docs/integrations/firebase) or [Firestore](/docs/integrations/firestore) database, using a freshly generated UUID as the document ID for each new user.
    * Generates a unique tracking ID for a customer support ticket and includes it in the subject line of all subsequent [Email](/docs/integrations/email) correspondence related to that ticket.
    * Processes a batch of items from a [Google Sheet](/docs/integrations/google-sheets), generates a unique ID for each one, and writes the data along with the new ID to a database.
