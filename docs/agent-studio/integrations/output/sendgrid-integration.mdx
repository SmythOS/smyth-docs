...
og:image: /img/integrations/smythos_sendgrid.png
...
|---------------|------------|------------------|------------|------------------|---------------|
| [**Send Dynamic Email**](#send-dynamic-email)| Write | Sends an email using a pre-built SendGrid Dynamic Transactional Template. | <Badge type="required" /> `from`, `to`, `template_id`, `JSON_template_handlebars` | `Response` | Sending personalized welcome emails or order confirmations. |
| [**Send Static Email**](#send-static-email) | Write | Sends a simple email with a subject and content defined directly in the agent. | <Badge type="required" /> `from`, `to`, `subject`, `content` | `Response` | Sending quick system alerts or basic notifications. |
...
* **Explore Other Integrations:**
    * Combine SendGrid with the [Firebase Integration](/docs/integrations/firebase). When a new user is created in Firebase Auth, trigger an agent to send them a welcome email.
    * Use an [LLM component](/docs/integrations/openrouter) to generate a personalized sales outreach message, then send it via the `Send Static Email` component.
    * Fetch data from a [Google Sheet](/docs/integrations/google-sheets) to power a mail merge, sending a dynamic email to each person listed in the sheet.
