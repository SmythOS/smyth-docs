...
description: Use the visual editor, SDK, CLI, and custom components to design, monitor, and extend agents in the SmythOS Runtime Environment.
...
The SmythOS Runtime Environment (SRE) gives you a powerful, flexible foundation for building sophisticated AI agents. You can interface with the runtime using the high-level **Studio** for visual design, the **Smyth SDK** for programmatic control, or the **SRE CLI** for project scaffolding and local execution.
