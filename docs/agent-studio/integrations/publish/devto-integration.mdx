...
og:image: /img/integrations/smythos_devto.png
...
* **Build an Agent That...**
    * Connects to your [GitHub](/docs/integrations/github) repository. When you merge a new Markdown file into your `blog` folder, the agent automatically takes the content and creates a draft on Dev.to.
    * Generates a weekly "Top 5 AI News" summary using an [LLM component](/docs/integrations/openrouter) and a [search tool](/docs/integrations/tavily), then publishes the summary as a blog post.
    * Connects to your [YouTube channel](/docs/integrations/youtube). When you publish a new video tutorial, the agent uses a speech-to-text model to get the transcript, formats it as a tutorial article, and saves it as a draft on Dev.to.

* **Explore Other Integrations:**
    * After an agent publishes a new article using `Publish Blog`, it can take the `articleUrl` and share it across all your social channels using the [Ayrshare Integration](/docs/integrations/ayrshare).
    * Keep a log of all published articles by having your agent write the `articleId` and `articleUrl` to a [Google Sheet](/docs/integrations/google-sheets).
