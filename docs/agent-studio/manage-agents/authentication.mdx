...
- 1. **Get Agent’s Subdomain** from Agent Studio settings.
...
1. **Log in to Your OAuth Provider’s Dashboard**: Access your OAuth provider's platform.
...
1. **Invalid Callback URL**
  ...
  - • Check for exact match in provider settings
  - • Ensure subdomain is correct
2. **Authentication Failures**
  - • Verify client ID and secret
  - • Check allowed email configuration
  - • Validate OIDC endpoint accessibility
3. **Bearer Token Issues**
  - • Confirm token format in header
