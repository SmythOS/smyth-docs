...
<InfoCallout title="Why this matters">You can reach nearly any REST-style service without custom code. <br/>Define your call once, pass dynamic inputs at runtime, and inject secrets via [Vault](/docs/agent-collaboration/working-with-agents/vault).</InfoCallout>
...
<InfoCallout>
Store API keys in [Vault](/docs/agent-collaboration/working-with-agents/vault) and inject them like `{{vault.api_key}}` inside headers or body.
</InfoCallout>
...
- Never hardcode credentials — use [Vault](/docs/agent-collaboration/working-with-agents/vault)  
