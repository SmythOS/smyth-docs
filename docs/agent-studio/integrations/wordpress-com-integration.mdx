...
og:image: /img/integrations/smythos_wordpress.png
...
* **Build an Agent That...**
    ...
    * Generates articles with an [LLM component](/docs/integrations/openrouter), then uses `Create Post` to save them as drafts for human review and publication.

* **Explore Other Integrations:**
    * After an agent publishes a new post, it can take the post URL from the output and share it across your social channels using the [Ayrshare Integration](/docs/integrations/ayrshare).
    * Keep a content inventory by having your agent log the ID and URL of every new post to a [Google Sheet](/docs/integrations/google-sheets) or [Notion](/docs/integrations/notion) database.
