...
og:image: /img/integrations/smythos_slack.png
...
* **Build an Agent That...**
    * Creates a daily "digest" of all new tasks created in [Asana](/docs/integrations/asana) or [Trello](/docs/integrations/trello) and posts it in a project channel.
...
* **Explore Other Integrations:**
    * Combine Slack with a database integration like [Firestore](/docs/integrations/firestore). When a new critical error is logged, trigger an agent to post the details to Slack.
    * Use an [LLM component](/docs/integrations/openrouter) to summarize a long document, then post the summary to a Slack channel for the team to review.
    * When a new lead is created in [HubSpot](/docs/integrations/hubspot), notify the assigned sales representative via a direct message in Slack.
