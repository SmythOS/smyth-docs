...
Never paste your AWS keys directly into these fields. Always store them securely in the **[Vault](../../agent-collaboration/working-with-agents/vault)** and reference them here as variables (e.g., `{{vault.aws_access_key}}`).
</TipCallout>
...
- Connect this to the output of an **[API Call Component](..//docs/agent-studio/components/base/api-call)** to perform a custom transformation on the response data that is too complex for a simple filter.
