...
- **Use for Initial Research:** `Web Search` is excellent for the first step of a research task. Use it to find relevant URLs, then pass those URLs to the **[Web Scrape Component](./web-scrape)** for deep content extraction.
...
- **Search Fails:** In rare cases, the search provider might be temporarily unavailable. You can build retry logic into your workflow using a **[Condition Component](../logic/logical-and)** to check for an empty result and loop back if needed.
</InfoCallout>
...
- Take the `Results` array and pass it to a **[ForEach Loop](../advanced/for-each)**. Inside the loop, use the **[Web Scrape Component](./web-scrape)** on each URL to extract its full content.
- Use the `Answer` output and feed it directly into an **[API Output Component](../base/api-output)** to create an agent that can answer questions with sourced information.
- Use the search results to feed a RAG pipeline. Scrape the content of the top results and use **[RAG Remember](../rag-data/rag-remember)** to add the new information to your agent's knowledge base.
