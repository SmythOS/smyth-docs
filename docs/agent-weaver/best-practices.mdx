...
<InfoCallout>
**TL;DR:** Build small ➜ test fast ➜ deploy in stages.  Everything else is icing. (There's plenty...)
</InfoCallout>
...
<FlowBar items={['Build small', 'Test early', 'Deploy staging', 'Ship to prod']} />
...
- Turn patterns into **Templates** (see 👉 <DocLink to="../agent-templates">Agent Templates</DocLink>).  
...
| Tip | Why |
|-----|-----|
| Store secrets in **Vault** | Encrypted, masked in logs |
| Apply **least-privilege** roles | Viewers read, Editors tinker |
| Rotate API keys **quarterly** | Block ex-employees & bots |
...
| Optimise | How |
|----------|-----|
| **Cache** static prompts | The Note component is free|
| **Choose models wisely** | GPT-4o mini beats full GPT-4o on price |
| **Throttle loops** | Drop an *FSleep* after 10 iterations |
