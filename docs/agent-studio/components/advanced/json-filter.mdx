...
- Chain an **[API Call](../advanced/api-call)** into a `JSON Filter` to immediately clean the response.
- Pass the filtered `Output` to a **[GenAI LLM Component](../base/gen-ai-llm)** to have it summarize only the essential information.
- Use the filtered object as the input for a **[ForEach Component](./for-each)** to loop through a cleaned list of items.
