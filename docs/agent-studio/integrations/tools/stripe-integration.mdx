...
og:image: /img/integrations/smythos_stripe.png
...
* **Build an Agent That...**
    * Manages the full new user lifecycle. When a user signs up in [Firebase Auth](/docs/integrations/firebase), the agent creates a corresponding customer in Stripe and a user profile in your [Firestore](/docs/integrations/firestore) database.
    * Automates invoicing. The agent retrieves a list of monthly active users, generates an invoice for each one using the Stripe API (via a custom API component), and then sends them an email notification using [Resend](/docs/integrations/resend).
    * Handles failed payments. When a subscription payment fails (triggered by a Stripe webhook), an agent sends a notification to the customer via SMS using [Twilio](/docs/integrations/twilio) and creates a follow-up task in [Asana](/docs/integrations/asana) for the customer success team.
...
* **Explore Other Integrations:**
    ...
    * Log every new Stripe customer or transaction to a [Google Sheet](/docs/integrations/google-sheets) for custom, real-time financial reporting.
