...
og:image: /img/integrations/smythos_discord.png
...
|---------------|------------|------------------|------------|------------------|---------------|
| [**Send Message**](#send-message) | Write | Sends a standard text message to a specified channel. | <Badge type="required" /> `channel_id`, `content` | `id`, `timestamp` | Sending simple notifications or alerts. |
| [**Send Message with Embed**](#send-message-with-embed) | Write | Sends a message with a rich, formatted embed. | <Badge type="required" /> `channel_id`<br /><Badge type="optional" /> `content`, `Body` | `id`, `timestamp` | Posting detailed reports or announcements. |
...
| **Post a structured, visually appealing announcement** | A channel ID and a formatted body | [Send Message with Embed](#send-message-with-embed) | Embeds allow for titles, colors, fields, and images, making your message stand out. |
...
| `content` | string | Yes | The text content of the message. Supports Discord's Markdown. |
...
### <a name="send-message-with-embed"></a>Send Message with Embed
Sends a message that includes a rich, formatted embed object.

<InfoCallout>This component requires a `Bot Token` for authentication, as detailed in the [Getting Started](#getting-started-with-discord) section.</InfoCallout>

#### Component-Specific Settings
- **Body:** A JSON editor where you define the structure of the embed object(s). This includes fields like `title`, `description`, `color`, `fields`, `footer`, `thumbnail`, etc.

#### Inputs
| Field | Type | Required | Notes |
|-------|------|----------|-------------|
| `channel_id` | string | Yes | The ID of the Discord channel where the message will be sent. |
| `content` | string | Optional | Standard text content that can appear above the embed. |
...
| `id` | string | The unique ID of the message that was sent. |
| `timestamp` | string | The ISO 8601 timestamp of when the message was sent. |
| `Response`| object | The full, raw JSON response from the Discord API, containing the complete message object with the embed. |
| `Headers` | object | The HTTP headers from the API response. |

<TipCallout title="Use Case" collapsible>
An agent fetches the latest release notes from GitHub. It then uses this component to post a beautifully formatted announcement in the `#updates` channel, with the release version as the title, the notes as the description, and a custom color.
</TipCallout>

```json
{
  "component": "discord.sendMessageWithEmbed",
  "channel_id": "123456789012345678",
  "content": "New Release Alert!",
  "Body": {
    "embeds": [
      {
        "title": "Version 2.5.0 Deployed!",
...
<WarningCallout title="Embed Structure">
The `Body` must be a valid JSON object containing an `embeds` key, which is an array of embed objects. Refer to the official [Discord Embed Object documentation](https://discord.com/developers/docs/resources/channel#embed-object) for all available fields.
</WarningCallout>
...
* **Use Embeds for Clarity:** For any data-rich notification, use the `Send Message with Embed` component. Embeds are much easier to read than long strings of plain text and allow for better visual organization of information.
...
* **Embed Does Not Appear Correctly**
  * **Cause:** The JSON structure in the `Body` of the "Send Message with Embed" component is invalid.
  * **Solution:** Carefully check your JSON syntax. Ensure it starts with `{"embeds": [...]}`. Use a JSON validator to check your structure against the official Discord documentation.
...
* **Build an Agent That...**
    ...
    * Fetches the latest release from a [GitHub repository](/docs/integrations/github) and posts a detailed changelog announcement using a rich embed.
...
* **Explore Other Integrations:**
    * Use the results from any search or data API (like [Tavily](/docs/integrations/tavily) or [Ahrefs](/docs/integrations/ahrefs)) to generate reports that are delivered directly to a Discord channel.
