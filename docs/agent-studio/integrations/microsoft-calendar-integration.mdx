...
* **Enhance Your Workflows by Combining Data & Actions:**
    * Use the [Microsoft Teams Integration](/docs/integrations/microsoft-teams) to send customized meeting reminders with additional context into a team channel.
    ...
    * Update a central dashboard in a [Google Sheet](/docs/integrations/google-sheets) or SharePoint with details of important meetings or calendar events managed by your agents for reporting or team visibility.
    * If an event is created for a customer meeting, an agent could also create a reminder task in [Asana](/docs/integrations/asana) or [Trello](/docs/integrations/trello).
