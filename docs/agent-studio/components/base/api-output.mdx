...
Use the **API Output** component to structure and standardize the final output of a workflow. It's especially powerful when used as the final step in an **[Agent Skill](../base/agent-skill)**, ensuring that any workflow acting as an API endpoint returns a clean, predictable, and well-formed JSON response.
...
- Make this the final component in an **[Agent Skill](./agent-skill)** to define its return value.
- Create two `API Output` components in your workflow and use a **[Condition](../logic/condition)** to route to one for a "success" response and the other for a "failure" response.
