...
Use the **Filestore** component to store binary content,such as images, CSVs, or files generated in `Code` or returned from an `API Call` and expose it through a public, time-limited URL. This makes it easy to share outputs with users or downstream systems outside SmythOS.
