...
With SmythOS, you can either connect directly via components or use the [Model Context Protocol (MCP)](/docs/agent-deployments/deployments/deploy-with-mcp) to serve your agent over a webhook server, which Telegram bots can poll for messages.
...
- Optionally, [Vault access](/docs/agent-collaboration/working-with-agents/vault-sharing) for securely storing tokens
...
If you want Telegram to **receive user messages and trigger agent replies**, deploy your agent with [MCP](/docs/agent-deployments/deployments/deploy-with-mcp):
...
4. Optionally store it in [Vault → API Keys](/docs/agent-collaboration/working-with-agents/vault-sharing)
...
> Tip: You can use [ElevenLabs](/docs/agent-deployments/integration-componenets/elevenlabs) to generate voice from text and pass the result here.
...
- Monitor your message or audio delivery in [Agent Logs](/docs/agent-studio/agent-settings/logs)
