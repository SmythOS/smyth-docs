...
SRE (Smyth Runtime Environment) is the operating system for AI agents: a modular, production-grade runtime that handles infrastructure, security, and orchestration so you can focus entirely on building agents.

Your AI agent’s performance, security, and reliability depend on how well its runtime handles key tasks like data access, AI model calls, and process management. SRE breaks these down into clear, manageable parts so your agents run smoothly at any scale.

You can build on SRE however you like:
- Use the [**Studio visual builder**](/docs/agent-studio/overview) to drag, drop, and connect components  
- Use the [**Smyth SDK**](/docs/agent-runtime/sdk-guide) for full programmatic control in TypeScript  
- Or combine both for hybrid workflows that balance no-code and code

<InfoCallout title="Why this architecture matters">
Your AI agents deserve more than a wrapper around an LLM API. SRE is built to make them secure, scalable, and deeply integrated with real-world data, workflows, and infrastructure.
</InfoCallout>
<Divider />
