...
- **Bearer Token Authentication:**
  ...
  - All incoming requests must include: `Authorization: Bearer your-token`.
...
See [Authentication Setup](/docs/agent-studio/agent-settings/authentication) for full details.
...
## Step 4: Handle Incoming Payloads in Your Agent

Configure these components to process incoming webhook data:
...
- Add [logging](/docs/agent-studio/agent-settings/logs) to monitor and debug webhook activity
- Consider [MCP deployment](/docs/agent-deployments/deployments-deploy-with-mcp) for higher control and flexibility
