...
keywords: [Logic, OR, condition, boolean, true, false, any, at least one]
...
- Use `Logic OR` in combination with **[Logic AND](./logical-and)** to build complex conditional flows, such as "(Condition A is true OR Condition B is true) AND Condition C is true."
- Use the output of a **[ForEach Loop](../advanced/for-each)** to check if at least one item in a list met a certain criteria.
