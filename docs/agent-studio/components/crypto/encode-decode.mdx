...
- Take the output from an **[Image Generator](../base/image-generator)**, `base64` encode it, and send it as part of a JSON payload in an **[API Call](/docs/agent-studio/components/base/api-call)**.
- Receive a `base64url` encoded token from an API response, decode it, and then use a **[JSON Filter](../advanced/json-filter)** to parse the resulting JSON object.
- Use the **[Hash Component](./hash)** on a piece of data, then `hex` encode the resulting hash for display or storage.
