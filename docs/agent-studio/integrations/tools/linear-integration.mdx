...
og:image: /img/integrations/smythos_linear.png
...
|---------------|------------|------------------|------------|------------------|---------------|
| [**Create Issue**](#create-issue) | Write | Creates a new issue in a specified team's backlog. | <Badge type="required" /> `title`, `description`, `teamId` | `issueId` | Automatically creating a bug report from a customer ticket. |
| [**List Teams**](#list-teams) | Read | Retrieves a list of all teams in your Linear workspace. | <Badge type="required" /> `trigger` | `teamId`, `teamName`| Finding the correct team ID to create an issue in. |
...
|-----------------|--------|--------------------|---------------|
| **Create a new bug report or feature request** | A `teamId` and a `title` | [Create Issue](#create-issue) | The primary method for programmatically adding new items to your Linear boards. |
| **Find the ID for your "Engineering" or "Design" team** | Your workspace | [List Teams](#list-teams) | This is an essential first step to get the `teamId` required to create issues. |
...
* **Build an Agent That...**
    ...
    * Monitors a [GitHub repository](/docs/integrations/github). When a new pull request is opened that mentions a specific issue ID in its title (e.g., `[LIN-123]`), the agent finds that issue in Linear and posts a comment with a link to the PR.
...
* **Explore Other Integrations:**
    * When a new issue is created in Linear, trigger an agent to create a corresponding channel in [Microsoft Teams](/docs/integrations/microsoft-teams) or [Slack](/docs/integrations/slack) for discussion.
