...
ElevenLabs offers some of the most realistic voice synthesis on the market — perfect for:
...
- Optionally configured Vault access in [Vault → API Keys](/docs/agent-collaboration/working-with-agents/vault-sharing) for managing secrets securely
...
<InfoCallout>
For secure storage and reuse, we recommend saving your API key in [Vault → API Keys](/docs/agent-collaboration/working-with-agents/vault-sharing) and to not share it with others.
</InfoCallout>
