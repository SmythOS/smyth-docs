...
- **Process Results with a Loop:** If you scrape multiple URLs, connect the `Results` output to a **[ForEach Loop](../advanced/for-each)** to process each scraped page individually.
...
- Scrape an article and feed the `Results` into a **[GenAI LLM Component](../base/gen-ai-llm)** to automatically generate a summary.
- Create a research agent that takes a topic, uses **[Web Search](./web-search)** to find a list of relevant URLs, and then uses `Web Scrape` to extract the content from each one.
- Build a knowledge base ingestion pipeline. Scrape a list of your company's blog posts and use **[RAG Remember](../rag-data/rag-remember)** to add the content of each post to your agent's memory.
