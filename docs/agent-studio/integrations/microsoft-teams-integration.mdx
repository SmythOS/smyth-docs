...
* **Explore Other Integrations:**
    * Combine Teams with the [tl;dv Integration](/docs/integrations/tldv). After a meeting, an agent can fetch the summary and post it as a reply in the original meeting's chat thread.
    * Use an [LLM component](/docs/integrations/openrouter) to summarize a long email, then post the summary to a Teams channel for quick review.
    * When a new file is uploaded to [OneDrive](/docs/integrations/onedrive), post a notification message with a link to the file in the appropriate team channel.
