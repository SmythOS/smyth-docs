...
| **Key/Secret** | Define the cryptographic key to use for signing. For **HMAC**, this is a secret string. For **RSA**, this must be a valid RSA private key (e.g., in PEM format). <TipCallout title="Secure Your Keys">Store your keys and secrets in the **[Vault](/docs/agent-collaboration/working-with-agents/vault-sharing)** and reference them here as variables (e.g., `{{vault.hmac_secret}}`).</TipCallout> |
...
- Use this component to generate a signature for an outgoing **[API Call](../base/api-call)**, adding the signature to the request headers.
...
- Take a hash generated by the **[Hash Component](./hash)** and sign that hash for an extra layer of verification.
