...
og:image: /img/integrations/smythos_playht.png
...
* **Build an Agent That...**
    * Takes the text from a scraped web article ([ScrapingBee](/docs/agent-studio/integrations/scrapingbee-integration)), converts it to audio with PlayHT, and saves the MP3 file to [OneDrive](/docs/integrations/onedrive).
    * Generates a personalized daily news briefing by getting headlines from [NewsAPI](/docs/integrations/newsapi) and reading them aloud.
    * Creates dynamic voice responses for a chatbot, using an LLM like [OpenRouter](/docs/integrations/openrouter) to generate the text and PlayHT to generate the audio.
