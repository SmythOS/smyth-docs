...
og:image: /img/integrations/smythos_medium.png
...
|---------------|------------|------------------|------------|------------------|---------------|
| [**Publish Blog**](#publish-blog) | Write | Publishes an article directly to a user's profile or publication. | <Badge type="required" /> `title`, `content`<br/><Badge type="optional" /> `userId` | `blogId`, `blogUrl` | Syndicating content from another blog. |
...
|-----------------|--------|--------------------|---------------|
| **Publish a new article** | A title and content | [Publish Blog](#publish-blog) | The core component for programmatically publishing content to Medium. |
...
<TipCallout title="Use Case">
This component is primarily used as a first step to get your `userId`, which you can then pass into the `Publish Blog` component to explicitly set the author.
</TipCallout>
...
* **Publishing to Publications:** To publish to a specific Medium Publication you own or contribute to, you first need to get that publication's ID. You can do this by using a custom API call to list the user's publications, then passing that ID into an advanced input field for the `Publish Blog` component.
...
* **Build an Agent That...**
    * Connects to your [Dev.to account](/docs/integrations/devto). When you publish an article there, an agent is triggered to re-publish the same content to Medium.
    ...
    * Connects to your [YouTube channel](/docs/integrations/youtube). When you publish a new video, the agent uses a speech-to-text model to get the transcript, formats it into an article, and publishes it to Medium to accompany the video.

* **Explore Other Integrations:**
    * After an agent publishes a new article to Medium, it can take the `blogUrl` output and share it across your social media channels using the [Ayrshare Integration](/docs/integrations/ayrshare).
    * Keep a log of all published articles by having your agent write the `blogId` and `blogUrl` to a [Google Sheet](/docs/integrations/google-sheets) or [Notion](/docs/integrations/notion) database.
