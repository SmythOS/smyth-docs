...
og:image: /img/integrations/smythos_agents.png
...
* **Explore Other Integrations:**
    * A parent agent can fetch data from a [Google Sheet](/docs/integrations/google-sheets), then use a `Loop` component to pass each row to a subagent for individual processing.
    * A subagent could be dedicated to a specific integration, like [Ahrefs](/docs/integrations/ahrefs). This "Ahrefs Specialist" agent could be called by any parent agent that needs to perform SEO analysis.
