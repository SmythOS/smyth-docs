...
- Take the `Results` array and pass it to a **[ForEach Loop](/docs/agent-studio/components/advanced/for-each)**. Inside the loop, use the **[Web Scrape Component](./web-scrape)** on each URL to extract its full content.
