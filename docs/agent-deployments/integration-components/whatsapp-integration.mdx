...
<InfoCallout> **TL;DR:** Choose from three WhatsApp integration paths: MCP Server, Meta WhatsApp Business API, or Twilio. All allow your SmythOS agent to receive and send messages over WhatsApp. </InfoCallout>
...
1. **[MCP Server Deployment](/docs/agent-deployments/deployments/deploy-to-mcp-server):** Recommended for control, branding, and reliability.
...
Follow the instructions in [Deploy to MCP Server](/docs/agent-deployments/deployments/deploy-to-mcp-server) to get your agent running on a stable, serverless endpoint.
...
5. **Generate Access Token** and store it in [Vault → API Keys](/docs/agent-collaboration/working-with-agents/vault-sharing)
...
2. **Copy your Account SID and Auth Token**, store in [Vault → API Keys](/docs/agent-collaboration/working-with-agents/vault-sharing)
...
- Add [logging for incoming messages](/docs/agent-studio/agent-settings/logs)
- Chain WhatsApp input into LLM-based responses or workflows
- Use [Vault](/docs/agent-collaboration/working-with-agents/vault-sharing) to manage auth securely
