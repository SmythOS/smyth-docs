...
Use the **[Await Component](../special/await)** only when your main workflow must pause and wait for a result from the async job before it can continue.
...
- Combine **`Async`** with the **[Await Component](../special/await)** to get results back in your main flow.
- Add a **[Send Email](../communication/send-email)** component at the end of your async branch to notify you when the job is complete.
- Use `Async` to wrap a **[For Each Loop](/docs/agent-studio/components/logic/for-each)** that processes thousands of records from a database without timing out.
- Store results from an async job in a permanent location using **[RAG Remember](../rag-data/rag-remember)** or by calling an external database.
