...
2. **resultPresignedImageUrl `required<string>`:** A presigned URL with `PUT` permissions where the resulting disparity map will be uploaded. For more information on generating presigned URLs, consult your cloud provider’s documentation.
