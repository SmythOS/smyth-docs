...
Each workflow is composed of [components](/docs/agent-studio/components), each designed to do one thing well. These might include:
...
If you want to loop through data, run tasks in parallel, pause steps, or store info, see all options in the [Logic, Flow & Memory Tools](/docs/agent-studio/components/overview#logic-flow--memory-tools) section.
</InfoCallout>
