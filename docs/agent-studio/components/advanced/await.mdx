...
Use the **Await** component to pause your main workflow until one or more background jobs, started by the **[Async Component](./async)**, have finished. This allows you to retrieve results from asynchronous tasks and use them in your main flow.
...
- **Combine with For Each:** After awaiting multiple jobs, use a **[For Each Loop](/docs/agent-studio/components/logic/for-each)** to process the `Results` array.
...
- Use a **[Code Component](../advanced/code)** after `Await` to parse the `Results` array and transform it into a more usable format.
- Feed the output from `Await` into a **[GenAI LLM Component](../ai-and-machine-learning/genai-llm)** to summarize the results from multiple background jobs.
