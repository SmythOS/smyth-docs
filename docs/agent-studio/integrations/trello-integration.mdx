...
og:image: /img/integrations/smythos_trello.png
...
* **Build an Agent That...**
    ...
    * Acts as a GitHub-to-Trello bridge. When a new issue is created in a [GitHub repository](/docs/integrations/github), the agent automatically creates a corresponding card on a "Development" Trello board.
    * Manages your content calendar. An agent reads a list of blog post ideas from a [Google Sheet](/docs/integrations/google-sheets) and creates a Trello card for each one in an "Ideas" list.

* **Explore Other Integrations:**
    * When a card is moved to the "Done" list on Trello (detected via webhook), trigger an agent to send a completion notification to a [Slack](/docs/integrations/slack) or [Discord](/docs/integrations/discord) channel.
    * Use an [LLM component](/docs/integrations/openrouter) to summarize a customer email, then create a Trello card with the summary as the description.
    * When a new client is added in [HubSpot](/docs/integrations/hubspot), create a new Trello card to track their onboarding process.
