---
title: Microsoft Outlook Integration with SmythOS
description: Seamlessly connect Microsoft Outlook Calendar with SmythOS to automate event creation, retrieve event details, and manage your calendar directly within your agent workflows.
og:title: SmythOS & Microsoft Outlook Calendar - Master Your Schedule with Automation
og:description: Learn how to integrate Microsoft Outlook Calendar with SmythOS. Use components to create calendars, manage events, and automate scheduling through your agents.
og:image: /img/integrations/smythos_microsoft_outlook.png
keywords: [Microsoft Outlook SmythOS Integration, Outlook Calendar SmythOS, Connect Outlook Calendar to SmythOS, SmythOS Outlook Calendar Components, Automate Outlook Calendar, SmythOS Microsoft 365 Calendar Automation]
---

# Integrating Microsoft Outlook Calendar with SmythOS

This page shows you how to connect Microsoft Outlook Calendar to SmythOS and pick the right component for every calendar and scheduling task.

> **Want your Outlook Calendar to work smarter, not harder?** Connect SmythOS to Microsoft Outlook Calendar to empower your agents to create calendars, manage events, and automate scheduling, bringing powerful organizational capabilities directly into your operational workflows.

<InfoCallout title="TL;DR"> Link your Microsoft Account to SmythOS by setting up OAuth 2.0 credentials in the Microsoft Entra (Azure AD) portal. Then, use our Microsoft Outlook Calendar components to enable your agents to automate various calendar tasks, reducing manual scheduling and event management.</InfoCallout>

**Quick Links:**
* [Why Integrate Microsoft Outlook Calendar?](#why-integrate-microsoft-outlook-calendar)
* [Getting Started: Prerequisites & Connection](#prerequisites-what-youll-need-to-get-started)
* [Components Overview](#microsoft-outlook-calendar-components-an-overview)
* [Understanding Common Fields](#understanding-common-outlook-calendar-component-io-fields)
...
Microsoft Outlook Calendar is a critical tool within the Microsoft 365 suite, offering robust features for managing schedules, appointments, and events. By integrating it with SmythOS, you empower your agents to interact with your calendar programmatically, unlocking significant automation potential.

- **Automated Calendar & Event Management**: Agents can create new calendars for specific projects or teams, schedule meetings, block out time, or create reminders in Outlook Calendar based on triggers from other systems or agent logic.
...
- **Centralized Scheduling Operations**: Manage all your Outlook Calendar scheduling needs from within SmythOS, reducing the need to switch between multiple applications and streamlining workflows.
...
Essentially, this integration transforms your Microsoft Outlook Calendar from a manual scheduling tool into an intelligent, interactive part of your SmythOS ecosystem. The result? Less manual effort, fewer scheduling errors, timely information, and more capacity for your team to focus on strategic tasks.

<InfoCallout>
<details>
  <summary>Why Integrate Microsoft Outlook Calendar with Your Agent?</summary>

Integrating Microsoft Outlook Calendar with SmythOS allows your agents to become active participants in your organization's scheduling and time management processes. Each component is designed for a specific calendar operation:
...
- **Automated Event Creation:** Let agents automatically schedule project kick-offs, client follow-ups, or internal reminders directly in team members' Outlook calendars when certain conditions are met in your workflows.
...
- **Centralized Logging of Time-Sensitive Tasks:** If your agent manages tasks with deadlines, it can create corresponding events in Outlook Calendar to ensure visibility and timely reminders for the team.

By connecting Microsoft Outlook Calendar, you reduce manual data entry for scheduling, minimize conflicts (with smarter future components), and ensure that team calendars accurately reflect activities and commitments managed by your automated SmythOS agents.
</details>
</InfoCallout>
...
- A **Microsoft Account** (Work or School account, or a personal Microsoft account, depending on your setup and API permissions) with access to Microsoft Outlook Calendar and permissions to grant access to applications.
- You will need to generate **OAuth 2.0 credentials (`Client ID` and `Client Secret`)** from the Microsoft Entra admin center (formerly Azure Active Directory portal). This process also involves configuring an OAuth consent screen and granting API permissions for Microsoft Graph (which includes Outlook Calendar). Detailed steps are provided below.
...
## Step-by-Step: Connecting Your Microsoft Outlook Account (for Calendar Access)

Let's establish the connection between SmythOS and your Microsoft Outlook Calendar. This involves obtaining OAuth 2.0 credentials from Microsoft Entra and then using them to authenticate within SmythOS. This is typically a one-time setup per Microsoft account you wish to integrate.
...
3.  **Provide Application Details:**
    * **Name**: Enter a descriptive name for your application (e.g., "SmythOS Outlook Calendar Integration").
...
5.  **Generate a Client Secret:**
    ...
    * **Description**: Provide a description for the secret (e.g., "SmythOS Outlook Integration Secret").
...
2.  **Add or Configure Microsoft Outlook (Calendar) Integration:**
    * Look for "Microsoft Outlook," "Microsoft 365," or a specific "Microsoft Calendar" integration in the list and click to add or configure it.
...
5.  **Microsoft Authorization Flow:**
    ...
    * Microsoft will then display a consent screen, showing the name of your registered application (e.g., "SmythOS Outlook Calendar Integration") and the permissions it's requesting (based on the scopes you configured in Azure). Review these permissions.
    * Click `Accept` or `Allow` to authorize SmythOS to access your Microsoft Outlook Calendar data.
6.  **Redirection to SmythOS:**
    * After successful authorization, you'll be redirected back to SmythOS.
    * The Microsoft Outlook (Calendar) integration should now indicate that it's connected and authenticated. **Connection Confirmed!**
...
<Image src="/img/microsoft_calendar/screenshot1.png" alt="Microsoft Outlook Calendar Authentication Settings in SmythOS" />
_Image: Example of Authentication/Sign Out button in component settings._
<Divider />

## Microsoft Outlook Calendar Components: An Overview

With your Microsoft Account successfully linked via OAuth 2.0, you gain access to a suite of Microsoft Outlook Calendar components within SmythOS. These components are designed to automate various calendar and event management tasks.
...
|----------|-------------|---------|
| [Create Calendar](#create-calendar) | Write | Creates a new calendar in the user's Outlook account.<br /><Badge type="required" /> `userId`, `name` |
...
## Understanding Common Outlook Calendar Component I/O Fields

Many Microsoft Outlook Calendar components in SmythOS share common input parameters for identifying users or specific calendar entities, as well as common output structures for API responses from Microsoft Graph. This section defines these common elements to avoid repetition in the detailed component guides.

<Tabs>
  <TabItem value="common-inputs" label="Common Inputs">
      ...
      <table>
        ...
        <tbody>
          <tr>
            ...
            <td>string</td>
            <td><strong>Required for all listed components.</strong> The user's ID (GUID), user principal name (e.g., <code>adelev@contoso.com</code>), or <code>me</code> to indicate the authenticated user. Defaults to <code>me</code>.</td>
          </tr>
          <tr>
            <td><code>calendarId</code></td>
            <td>string</td>
            <td>Unique identifier of a specific calendar. Required for "Update Calendar" and "Delete Calendar". Can also target non-default calendars.</td>
          </tr>
          <tr>
            <td><code>eventId</code></td>
            <td>string</td>
            <td>Unique identifier for a specific event. Required for components like Get/Update/Delete Event.</td>
          </tr>
          <tr>
            <td><code>Body</code></td>
            <td>object</td>
            <td><strong>Required for "Update Calendar", "Create Event".</strong> A JSON object representing the calendar or event data.</td>
          </tr>
          <tr>
            <td><code>name</code></td>
            <td>string</td>
            <td><strong>Required for "Create Calendar".</strong> Name of the new calendar.</td>
          </tr>
        </tbody>
      </table>
    </div>
  ...
  <TabItem value="calendar-outputs" label="Common Outputs (Calendar Resource)">
    <div className="tab-content">
      <p>Fields from the Calendar resource (Microsoft Graph API):</p>
      <table>
        <thead>
          <tr>
            <th>Field</th>
        ...
        <tbody>
          <tr><td><code>id</code></td><td>string</td><td>Unique calendar ID.</td></tr>
          <tr><td><code>name</code></td><td>string</td><td>Calendar name.</td></tr>
          <tr><td><code>color</code></td><td>string</td><td>Pre-defined color value (e.g., <code>lightBlue</code>).</td></tr>
          <tr><td><code>changeKey</code></td><td>string</td><td>Version key that updates on changes.</td></tr>
          <tr><td><code>canEdit</code></td><td>boolean</td><td>True if user can edit the calendar.</td></tr>
          <tr><td><code>canShare</code></td><td>boolean</td><td>True if user can share the calendar.</td></tr>
          <tr><td><code>canViewPrivateItems</code></td><td>boolean</td><td>True if user can see private events.</td></tr>
          <tr><td><code>owner</code></td><td>object</td><td>Owner information (name and email).</td></tr>
        </tbody>
      </table>
    </div>
  </TabItem>

  <TabItem value="event-outputs" label="Common Outputs (Event Resource)">
    <div className="tab-content">
      <p>Fields from the Event resource (Microsoft Graph API):</p>
      <table>
        <thead>
          <tr>
            <th>Field</th>
        ...
        <tbody>
          <tr><td><code>id</code></td><td>string</td><td>Event ID.</td></tr>
          <tr><td><code>iCalUId</code></td><td>string</td><td>Unique recurring event identifier.</td></tr>
          <tr><td><code>subject</code></td><td>string</td><td>Event title.</td></tr>
          <tr><td><code>body</code></td><td>object</td><td>Message body (HTML/Text).</td></tr>
          <tr><td><code>start</code></td><td>object</td><td>Start datetime and timezone.</td></tr>
          <tr><td><code>end</code></td><td>object</td><td>End datetime and timezone.</td></tr>
          <tr><td><code>attendees</code></td><td>array</td><td>List of attendees.</td></tr>
          <tr><td><code>location</code></td><td>object</td><td>Main location info.</td></tr>
          <tr><td><code>locations</code></td><td>array</td><td>Additional location objects.</td></tr>
          <tr><td><code>webLink</code></td><td>string</td><td>URL to open in Outlook.</td></tr>
          <tr><td><code>isAllDay</code></td><td>boolean</td><td>True if all-day event.</td></tr>
          <tr><td><code>organizer</code></td><td>object</td><td>Organizer info (emailAddress object).</td></tr>
          <tr><td><code>recurrence</code></td><td>object</td><td>Recurrence pattern, null if one-time.</td></tr>
          <tr><td><code>createdDateTime</code></td><td>string</td><td>ISO 8601 timestamp (created).</td></tr>
          <tr><td><code>lastModifiedDateTime</code></td><td>string</td><td>ISO 8601 timestamp (last modified).</td></tr>
        </tbody>
      </table>
    </div>
  </TabItem>

  <TabItem value="general-outputs" label="General Outputs">
    <div className="tab-content">
      <table>
        <thead>
          <tr>
        ...
        <tbody>
          <tr><td><code>Response</code></td><td>object</td><td>Full raw JSON response from Microsoft Graph.</td></tr>
          <tr><td><code>Headers</code></td><td>object</td><td>HTTP response headers (e.g., <code>request-id</code>, <code>Retry-After</code>).</td></tr>
        </tbody>
      </table>
    </div>
  </TabItem>
...
## Deep Dive into Outlook Calendar I/O Fields

Switch between **Inputs** and **Outputs** below to see more detailed explanations and examples for parameters frequently used across multiple Microsoft Outlook Calendar components.
...
## Quick Reference: Microsoft Outlook Calendar Components

Use this table to quickly compare inputs, outputs, and typical use cases for each Microsoft Outlook Calendar component.
...
## Which Microsoft Outlook Calendar Component Should I Use With My Agent?
...
This section provides detailed information for each Microsoft Outlook Calendar component. Remember to refer to the [Understanding Common Outlook Calendar Component I/O Fields](#understanding-common-outlook-calendar-component-io-fields) and [Deep Dive Into Outlook Calendar I/O Fields](#deep-dive-into-outlook-calendar-io-fields) for explanations of common inputs (like `userId`, `calendarId`, `Body`) and outputs (like Calendar resources, Event resources, `Response`, `Headers`). Each component guide below will highlight specific inputs and outputs for that action.

### <a name="create-calendar"></a>Create Calendar

Create a new calendar in the default calendar group or a specified calendar group for a user in Microsoft Outlook Calendar.
...
To kickstart this component, refer to **Part 1, Step 1: Setting Up the Microsoft Outlook Integration (OAuth) in SmythOS** for detailed instructions on obtaining your `Client ID` and `Client Secret` using Microsoft Entra and configuring API permissions. Essential permissions typically include `Calendars.ReadWrite`.
...
```json
{{
  "component": "microsoftOutlook.createCalendar",
  "userId": "me",
  "Body": {`{{
...
Update the properties of an existing calendar object in Microsoft Outlook Calendar, such as its name or color.
...
```json
{`{{
  "component": "microsoftOutlook.updateCalendar",
  "userId": "me",
...
This component is used to delete a calendar object in Microsoft Outlook Calendar. This action is permanent.

<InfoCallout title="Use Case Example">
When a temporary project concludes, an agent automatically deletes the associated project calendar to keep the user's Outlook clean and organized.
</InfoCallout>

#### Settings
(Refer to the "Settings" section under [Create Calendar](#create-calendar) for details on Client ID/Secret and authentication.)
<Image src="/img/microsoft_calendar/screenshot7.png" alt="Microsoft Outlook Calendar Authentication Settings in SmythOS" />
...
```json
{`{{
  "component": "microsoftOutlook.deleteCalendar",
  "userId": "me",
...
Create a new event by posting to the events collection of a user's primary calendar or a specified calendar in Microsoft Outlook.

<InfoCallout title="Use Case Example">
An agent processes a new employee onboarding form and automatically schedules a "Welcome Meeting" in the new employee's Outlook calendar, inviting their manager and HR representative.
</InfoCallout>
...
| `webLink` | string | A direct link to view the event in Outlook on the web. Extracted from `Response.webLink`. |
...
```json
{{
  "component": "microsoftOutlook.createEvent",
  "userId": "me",
  "Body": {{
...
Retrieve a list of events from a user's primary calendar or a specified calendar in Microsoft Outlook. Supports filtering and pagination.
...
#### Settings
(Refer to the "Settings" section under [Create Calendar](#create-calendar) for details on Client ID/Secret and authentication.)
<Image src="/img/microsoft_calendar/screenshot13.png" alt="Microsoft Outlook Calendar Authentication Settings in SmythOS" />
_Image: Authentication settings, similar for all components._
...
```json
{{
  "component": "microsoftOutlook.listEvents",
  "userId": "me",
...
Encountering an issue with your Microsoft Outlook Calendar integration? Here are solutions to some common problems:
...
* **`404 Not Found` Errors (for Update/Delete Calendar, or specific Event operations):**
    ...
    * **Solution:** Double-check the ID for typos. Confirm the calendar or event exists in the user's Outlook. Verify permissions.
...
With SmythOS now integrated with your Microsoft Outlook Calendar, your agents are empowered to automate and streamline your scheduling, event management, and time-based workflows.

Here are some ideas to inspire your next automation:

* **Build an Agent That...**
    * Monitors a project management tool for new task assignments with deadlines and automatically creates corresponding events in the assignee's Outlook Calendar using "Create Event".
    ...
    * Sends a daily or weekly agenda summary to users via Microsoft Teams or email, by fetching upcoming events ("List Events") from their Outlook Calendar.
...
* **Enhance Your Workflows by Combining Data & Actions:**
    * Use [SmythOS Email Integration (Outlook or Gmail)](link-to-email-integration-doc) to send customized email notifications or meeting invitations with additional context beyond the standard Outlook invites.
    * Trigger calendar events based on data received from [SmythOS Database Connectors](link-to-database-integration-docs) or other third-party API integrations (e.g., creating a calendar event when a new sales lead reaches a certain stage in a CRM).
...
Explore the full potential of calendar automation with SmythOS and Microsoft Outlook!
