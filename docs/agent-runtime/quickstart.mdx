...
Build your first real AI agent with behavior, skills, and prompt flow using the [Smyth SDK](/docs/sre/sdk-guide).
</InfoCallout>
...
If you haven’t done that yet, head to [Getting Started with SRE](/docs/sre/getting-started) first.

<Spacer size="md" />

## Step 1: Create your first agent

Start by defining your agent’s name, model, and role in the conversation using `SDK.Agent`.
...
Skills are reusable functions your agent can call. Here’s one to summarize user issues using `agent.addSkill`:
...
Now try prompting the agent directly with `agent.prompt` and print the result. This is a simple one-shot interaction.
...
You can use a persistent `agent.chat()` to maintain conversational context across turns.
...
You can build simple workflows by connecting modular [Components](/docs/sre/components). This example uses `SDK.GenAILLM` and `SDK.Classifier`.
...
- [Learn how SRE's architecture works](/docs/sre/architecture)
