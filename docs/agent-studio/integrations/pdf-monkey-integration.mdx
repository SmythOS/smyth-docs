...
* **Build an Agent That...**
    * Generates monthly invoices. The agent fetches billing data from [Stripe](/docs/integrations/stripe-integration), populates an invoice template using `Generate PDF`, and then emails it to the customer using [SendGrid](/docs/integrations/sendgrid).
