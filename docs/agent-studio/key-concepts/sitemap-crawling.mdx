...
3. Enter a suitable name for the data source as this is mandatory
4. Enter your sitemap URL (e.g., `https://example.com/sitemap.xml`) in the last field
<Image src="/docs/img/data_pool/data_spaces/add-sitemap-xml.png" alt="Sitemap XML format and where to enter it in Data Space SmythOS"/>
5. Click **Add Data Source* for it to start indexing.
...
Use [RAG Search](/docs/agent-studio/components/rag-data/rag-search) to retrieve semantically relevant results from the sitemap content. Configure it to query the correct Data Space, adjust result count, and toggle metadata inclusion.

### RAG Remember

Enhance a sitemap-based Data Space with additional content like manually written summaries or notes using [RAG Remember](/docs/agent-studio/components/rag-data/rag-remember). This is useful when you want to enhance the crawled data.

### RAG Forget

Only manually added content can be removed using the [RAG Forget](/docs/agent-studio/components/rag-data/rag-forget) component. Sitemap-crawled content is persistent and cannot be deleted through workflows.
...
Use one [Data Space](/docs/agent-collaboration/data-pool/data-spaces) for each domain and add its sitemap. Best for simple websites or small teams.

### Multi-domain

Use separate [Data Spaces](/docs/agent-collaboration/data-pool/data-spaces) for each domain. Helps keep content clean and improves targeted search.
...
Name each [Data Space](/docs/agent-collaboration/data-pool/data-spaces) clearly to simplify long-term management. For example: `product-sitemap`, `blog-sitemap`, `support-content`.
</InfoCallout>
