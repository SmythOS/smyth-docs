...
* **Build an Agent That...**
    * Acts as a full product lifecycle manager. When a new product is added to a [Google Sheet](/docs/integrations/google-sheets), the agent creates it in InFlow, then creates a corresponding product page on a [Shopify](/docs/integrations/shopify) or [Squarespace](/docs/integrations/squarespace) store.
