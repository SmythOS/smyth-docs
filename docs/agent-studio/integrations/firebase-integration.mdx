...
og:image: /img/integrations/smythos_firebase.png
...
* **Explore Other Integrations:**
    * Combine Firebase Auth with a database integration like [Firestore (via the OpenAPI component)](/docs/integrations/openapi) or a SQL database to store and retrieve data associated with a user's `localId`.
    * After a new user is created, trigger a welcome [Email](/docs/integrations/email) to be sent to their registered email address.
